...
- [ ] T009 [US1] Add pure XML-template generator in `src/KbFix/Platform/Install/ScheduledTaskRegistry.cs` (static `BuildTaskXml(string stagedBinaryPath, string userSid)` method) — emits the full Task Scheduler XML with `<LogonTrigger>` (for the current user SID), `<Principal LogonType=InteractiveToken RunLevel=LeastPrivilege>`, `<Settings><MultipleInstancesPolicy>IgnoreNew</MultipleInstancesPolicy><ExecutionTimeLimit>PT0S</ExecutionTimeLimit><StartWhenAvailable>true</StartWhenAvailable></Settings>`, and `<Actions><Exec><Command>` pointing at the staged binary with `<Arguments>--watch</Arguments>`; structurally matches research R1.
- [ ] T010 [P] [US1] Add new step records `CreateScheduledTaskStep(string xmlPath)`, `DeleteScheduledTaskStep`, `ExportScheduledTaskXmlStep(string destPath)` to `src/KbFix/Watcher/InstallDecision.cs` beside the existing step-record hierarchy.
- [ ] T011 [US1] Wire the new step types into `src/KbFix/Platform/Install/InstallExecutor.cs` — add `switch` cases that delegate to `ScheduledTaskRegistry.Create`, `ScheduledTaskRegistry.Delete`, and (for `ExportScheduledTaskXmlStep`) `File.WriteAllText(path, ScheduledTaskRegistry.BuildTaskXml(...))`; maintain the 003 pattern of returning `StepResult` with a `Note` string for the reporter.
- [ ] T012 [US1] Create `SupervisorDecision` in `src/KbFix/Watcher/SupervisorDecision.cs` — pure decision function `AppendInstallSteps(List<InstallStep>, WatcherInstallation state, string invokingBinaryPath)` and `AppendUninstallSteps(List<InstallStep>)` that emit the new step types per the decision table in `research.md` §R8 and `data-model.md` §2; also exposes `ClassifySupervisor(WatcherInstallation) -> SupervisorState` per `data-model.md` §3.
- [ ] T013 [US1] Update `InstallDecision.ComputeInstallSteps` and `ComputeUninstallSteps` in `src/KbFix/Watcher/InstallDecision.cs` to invoke `SupervisorDecision.AppendInstallSteps` / `AppendUninstallSteps` after the existing 003 steps — preserves the 003 invariant that the Run key is written first, the task second.
- [ ] T014 [US1] Extend `WatcherDiscovery.Probe` in `src/KbFix/Platform/Install/WatcherDiscovery.cs` to populate the new `ScheduledTask` field via `ScheduledTaskRegistry.QueryXml` + `QueryVerbose`, and the new `SupervisorState` field via `SupervisorDecision.ClassifySupervisor(this)`; preserve all existing 003-derived fields.
- [ ] T015 [US1] Update `InstallReporter.cs` in `src/KbFix/Cli/InstallReporter.cs` — `FormatInstall` and `FormatUninstall` must emit the new `task:` line per `contracts/cli.md` §`--install` and §`--uninstall`, with wording that handles absent / stale / created / deleted / denied-by-policy states.
- [ ] T016 [P] [US1] Unit tests for `SupervisorDecision` in `tests/KbFix.Tests/Watcher/SupervisorDecisionTests.cs` — exhaustive decision-table coverage matching `research.md` §R7: fresh install (no task, no run key), 003-upgrade-case-A (run key present, task absent, watcher alive), 003-upgrade-case-B (new binary in Downloads, watcher alive from old binary), task present + disabled, task stale-path, idempotent re-install. Use the same fake-`WatcherInstallation`-building helpers the 003 `InstallDecisionTests` already establish.
- [ ] T017 [P] [US1] Unit test for `ScheduledTaskRegistry.BuildTaskXml` in `tests/KbFix.Tests/Platform/ScheduledTaskRegistryTests.cs` — asserts the emitted XML contains the expected `<UserId>`, `<LogonType>InteractiveToken</LogonType>`, `<RunLevel>LeastPrivilege</RunLevel>`, and `<Command>` pointing at the supplied staged binary; asserts it does NOT contain `HighestAvailable` under any input.
